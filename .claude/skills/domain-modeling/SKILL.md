...
<!-- Vendored from mattpocock/skills (MIT © 2026 Matt Pocock), skills/engineering/domain-modeling/SKILL.md.
...
The glossary is one file at the repo root; the decisions live beside it:
...
├── docs/
│   └── adr/
│       ├── README.md                 ← the format, the gate, and the index
│       ├── 0001-event-sourced-orders.md
│       └── 0002-postgres-for-write-model.md
└── src/
```

Create files lazily — only when you have something to write. If no `GLOSSARY.md` exists, create one when the first term is resolved. If no `docs/adr/` exists, create it when the first ADR is needed. Where either is missing, say which one and carry on rather than inventing a substitute location.
...
The mechanism lives in one place: `docs/adr/README.md` carries the body format, the numbering, the index table that has to be updated in the same commit, the lifecycle statuses, and the rule that the draft is presented and written **only on explicit approval**. Read it before drafting — this skill deliberately holds no second copy of the format.
