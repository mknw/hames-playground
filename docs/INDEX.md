...
| [plan/harness-npm-lib.md](plan/harness-npm-lib.md)                   | Extracting harness-patterns + companions (sandbox/stash/retriever/harness-client) into pnpm-workspace npm packages: proposed `packages/*` layout and dependency graph, the `baml_client`/`settings` coupling that must be broken first, `workspace:*` dev loading vs. registry-only production docker builds, a Changesets publish flow, and a risk-ordered migration                                                                                                       |
