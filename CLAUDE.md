...
The live check is manual (CI has no endpoint access): `USE_VERDA_INFERENCE=1 pnpm dlx tsx --env-file=.env src/lib/harness-patterns/scripts/smoke-verda.ts` from `app/` — two calls, back-to-back to pay one cold start, each asserting the collector reports `VerdaQwen` rather than trusting the flag, after a preflight that re-reads the served model id from `GET /v1/models` (the deployment, not this repo, decides what it serves). Its sibling `smoke-verda-load.ts` measures the same route under load — sequential baseline, 4- and 8-way concurrency, and 20 controller-shaped calls counted for parse failures; measured 2026-08-25 at a 2.75k-token prompt: 4.1s single-call, 6.3s p50 / 9.9s p95 at 8 in flight, 64 → 208 tok/s aggregate decode, 47/47 envelopes parsed. One replica, so concurrency is queueing rather than scaling. The hermetic half is two files under `app/src/__tests__/lib/harness-patterns/`: `clients-verda.test.ts`, which pins that every role in the Verda map has a call site spreading `clientOverrideFor(role)` — an entry without one reads like routing and changes nothing — and `verda-body-shape.test.ts`, which renders a request offline and pins what goes on the wire (the thinking kwarg, the model id, and no `cache_control` anywhere).
