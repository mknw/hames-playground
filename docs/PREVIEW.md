...
| "Inference runs on our own hardware."            | Unless you turn the switch on, every BAML chain routes to Anthropic — and even with it on, four roles stay there, so it is a routing switch and never a "nothing leaves the building" claim. The preview leaves it off.                                                                                  | **Shipped** — PR #263 merged 2026-08-26, so `main` carries it. `USE_VERDA_INFERENCE='1'` re-points the controller / actor / critic / synthesizer only; router, describe, screen and planner stay on Anthropic. Unset, as the template says to leave it, nothing about this deployment changes |
