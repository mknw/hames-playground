...
consulted — which tier a turn takes is decided by `resolveConversationTier()`
...
| `E2E_COLD_START_MS`             | `90000`                            | How long scenarios 4 and 8 withhold a self-hosted response. Keep it **under 180 000**: scenario 4 aims its delay past the wake ping, so at or above `VerdaQwen`'s own `request_timeout_ms` the client breaks its budget by design and the red is not a finding. Raising it also moves the wake's per-attempt bound with it — see the next row.                                                                                                                                                                                                                          |
| `VERDA_WAKE_ATTEMPT_TIMEOUT_MS` | derived: `E2E_COLD_START_MS + 30s` | **Not a knob — `bootApp` sets it** on the hermetic path, and the derivation is why raising `E2E_COLD_START_MS` stays safe. The wake is a poll, so its shipped 30s per-attempt bound would slice every injected delay into attempts and make scenarios 3, 4 and 8 measure the poll's cadence instead of the budget each is named for. Set above the delay, one attempt covers one wait. The poll's retry behaviour is not configured away: `verda-wake.test.ts` owns it on fake timers, and scenario 8's "keeps polling" test lowers this var itself. See `lib/mode.ts`. |
...
| `04-cold-start-survival.e2e.ts`    | A 90-second withheld self-hosted BAML response does not kill the turn — through the server action and through the SSE route, which holds a stream open across the whole wait. The delay is aimed PAST the wake ping (`wake: false`), so what it exercises is `VerdaQwen`'s own `request_timeout_ms` (180s) rather than the wake's bound; `E2E_COLD_START_MS` must therefore stay under 180 000.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `05-tier-switch.e2e.ts`            | Flipping a conversation's switch between turns moves both switched roles this chain uses (`LoopController` **and** `Synthesize`) and does not fork the conversation; and the cheap side-roles (`Router`, the title, the post-turn describe) follow the switch in BOTH directions, which is the only check that would catch a missing spread at one of the six describe call sites. `screen` is pinned by source scan instead — no agent enables the LLM screen, so no turn here makes one (`SA-M5` / `SD-4`).                                                                                                                                                                                                                                                                                                                        |
...
- **A gateway that dies MID-turn.** Scenario 9 takes it down before the turn, so
  ...
  fails inside `callTool` instead, and that is the path the loop is _supposed_ to
  survive — the controller sees the tool error and carries on (scenario 6's
  control case). The seam between the two is deliberate and is documented on
  `gateway-health.server.ts`; nothing here pins where it sits.
...
| `simpleLoop.server.ts`: the `toolSurfaceOutage` refusal removed                                               | 09, on all cases — `the turn said nothing about the gateway. status=done, response="E2E-FAKE-ANSWER: the graph reports 42 nodes."`, which is verbatim the silent shape #276 was filed for |
| `gateway-health.server.ts`: the guard opens with `tools.length > 0` again (its pre-#278 shape)                | 09's two `general` cases, with that same string — and NOT the `search` ones, which is the point: `search` is handed `[]` and `general` is handed nine surviving app-side tools            |
| `baml-adapters.server.ts`: a degraded catalog is cached again (the `gatewayDegradation()` early return)       | 09's control case — the gateway comes back, the controller is still shown no tools, picks one it was never offered, and the allowlist rejects it. Found BY adding the `general` cases     |
