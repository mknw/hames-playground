...
Two things re-point part of that, and both move exactly the roles in `VERDA_CLIENT_BY_ROLE` — which since 2026-08-26 is EVERY role: controller / actor / critic / synthesizer / router / planner / describe, and the injection `screen`. Two owner decisions that day, in order. Router / planner / describe were held out on latency grounds until the first overruled it (the router sees the user's raw message and describe is handed tool results verbatim, so excluding them left the private tier shipping exactly the payloads it exists to keep). `screen` was held out on security grounds — `VerdaQwen` is unmeasured as a screener — until the second overruled that too, on a rule rather than a rebuttal: **no call made under the private tier may be sent to any public AI provider** (answer 7; no exception sought). The measurement is owed, not waived: `pnpm eval:harness`'s `screen-on-the-tier` scenario grades it, and a failing run of that is evidence the move was wrong. The two levers:
...
Both act through the SAME seam: a per-call `client` override spread into the BAML options bag. Re-pointing the chains in `baml_src/` is not the mechanism and must not become it — that class of edit moves whole roles at once, and the `screen` role exists precisely so that moving the injection screen is always a DELIBERATE line rather than a side effect of moving summarization (SA-M5). The private tier now moves both, by owner decision; the accident it guards against is still live, because `screen` and `describe` name the same chain in BAML and the difference between them exists only in `clients.server.ts`.
