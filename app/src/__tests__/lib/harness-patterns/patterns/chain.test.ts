...
describe('runChain (internal executor)', () => {
  ...
  it('should execute patterns in sequence', async () => {
    ...
    const pattern2 = configurePattern(
      'second',
      async (scope) => {
    ...
    await runInFrame(() => runChain(ctx, [pattern1, pattern2]))
  ...
  it('should stop execution when status changes from running', async () => {
    ...
    const pattern3 = configurePattern(
      'third',
      async (scope) => {
    ...
    await runInFrame(() => runChain(ctx, [pattern1, pattern2, pattern3]))
  ...
  it('should handle errors in patterns', async () => {
    ...
    const ctx = createContext('test')
    await runInFrame(() => runChain(ctx, [errorPattern]))
...
describe('chain (pattern factory)', () => {
  ...
  it('should execute sub-patterns in sequence within scope', async () => {
    ...
    const result = await runInFrame(() => composed.fn(scope, view))
