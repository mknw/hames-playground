...
| 7   | `domain-modeling`               | `engineering/domain-modeling/`               | **model** — dependency target                  | Copy `SKILL.md` + `CONTEXT-FORMAT.md`; **replace `ADR-FORMAT.md`** with the reconciled format from §3.2. Drop the `CONTEXT-MAP.md` multi-context branch entirely — this repo is one `ui/` app, so it is a branch that can never fire (a pure context-load cost by `writing-for-agents`' own no-op test).                                                                                                                                                                                                                                                                    | M      |
...
| 9   | `resolving-merge-conflicts`     | `engineering/resolving-merge-conflicts/`     | **model**                                      | Copy verbatim (14 ln), then sharpen step 4 from "discover the project's automated checks" to name them: from `ui/`, `pnpm exec tsc --noEmit --project tsconfig.json`, `pnpm test:run`, `pnpm lint`, prettier on changed files. That is a cache of a non-obvious lookup (the `ui/` cwd requirement), so it earns its lines.                                                                                                                                                                                                                                                  | S      |
...
| Repo standards             | **new**                       | UnoCSS attributify only (never `class=`, except `i-*` icons); `.server.ts` + `assertServerOnImport()`; BAML functions always `.bind(b)`; never edit `baml_client/`; `pnpm` only, run from `ui/`; conventional commits; **no attribution trailers**; `CLIENT_MAX_OUTPUT_TOKENS` kept in sync with the BAML client `max_tokens` |
...
| **Commands block**                                  | Cache — `pnpm dev`, `pnpm build`, `pnpm test:run` are a one-file lookup in `ui/package.json`         | Keep only the non-obvious ones: the `ui/` cwd requirement, the two llama-server ports (8080 chat vs 8090 embeddings — the trap), `USE_MIXED_CHAINS=1`, and `baml-generate` after any `baml_src/` edit. Drop the rest                  |
...
2. `orca.yaml`'s `scripts.setup` copies the gitignored bits by hand: `ui/.env`,
