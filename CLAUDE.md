...
**Wrong envelope, right action:** a third recoverable parse cause, and the only one where the model was not wrong about the work. Captured live (`.harness-logs/baml-validation.json`): the sandbox actor wrote a COMPLETE action — reasoning, tool, escaped `tool_args`, a second call in `additional_calls`, status, `is_final` — as brace-less `key: value` lines, because its own few-shot section rendered examples in exactly that shape. 372 output tokens against a 32768 cap and a non-empty response, so neither existing retry trigger matched; the throw killed the actorCritic loop with three attempts still in budget. Fixed at both ends: `ActorFewShots` / `LoopFewShots` now render the JSON object envelope and the mode-gated `LoopMultiCalls` / `ActorMultiCalls` branches demonstrate a batch (fourth instance of "disagreeing demonstrations are defects" — see the comment on `LoopFewShots`), and `coerceControllerActionText` (`controller-action.ts`) recovers the shape in the adapters BEFORE the truncation branch, since no retry can improve on an action that is already correct. It declines anything it cannot read completely — a partly-readable `additional_calls`, an unterminated literal, or a second top-level `tool_name`/`tool_args`/`additional_calls` (an echoed tool result must never get to choose the call, and this runs upstream of `withInjectionGuard`) — so a half-batch is never executed. Truncation is decided by `collectorHitOutputCap`, NOT by the lexical unterminated-literal signal: the recovery declines on a cap-hit before it reads the text at all, because a cut can land inside a QUOTED `tool_args` or between two `additional_calls` items and leave nothing unbalanced to see. The 372/32768 capture is unaffected; a genuine cap-hit stays on the truncation retry.
