/**
 * The app-tool registry + the graph_me tool (#110).
 *
 * Asserts the security invariants from #107: the advertised schema carries no
 * credential field, the user id comes from the injected resolver (never args),
 * and a call outside any user scope is refused. Moved co-located with the
 * registry (#225 PR-C2); composed through the harness — the resolver is the
 * harness's scope, standing in for the host's request scope.
 */
import { describe, it, expect, vi, beforeEach } from 'vitest'
import { buildGraphHarness } from '../graph/harness'
import { shapeMe } from '../../graph/graph-tools.server'

const h = buildGraphHarness()
const { hasAppTool, runAppTool, appToolDescriptions, appToolNamespace, registerAppTool } = h
...
describe('runAppTool', () => {
  ...
  it('passes the request-scoped sessionId to the executor (not from args)', async () => {
    ...
    h.setScope({ sessionId: 'sess-real' })
