...
import { defaultSelector } from '@hames-ai/harness-baml/defaults.server'
import type {
  ContextEvent,
  PatternScope,
  ToolResultEventData,
...
function makeInnerPattern(name = 'inner'): {
...
describe('withReferences', () => {
  ...
  it('cache hit reuses the prior decision without re-calling selector', async () => {
    ...
    const scope1: PatternScope<Record<string, unknown>> = {
      id: 'wrap',
      data: {},
      events: [],
      startTime: Date.now(),
    ...
    const scope2: PatternScope<Record<string, unknown>> = {
      id: 'wrap',
      data: {},
      events: [],
      startTime: Date.now(),
    ...
    expect((scope2.data.attachedRefs as Array<{ ref_id: string }>).map((r) => r.ref_id)).toEqual([
      'ev-a',
    ])
  })
  ...
  it('excludes hidden, archived, and failed tool_results from candidates', async () => {
    ...
    const ctx = createContext<Record<string, unknown>>('test')
    ctx.events.push(
      toolResultEvent({ id: 'ev-good', patternId: 'p', tool: 'x', result: 'G', summary: 'good' }),
    )
    ctx.events.push(
      toolResultEvent({
        id: 'ev-hidden',
  ...
  it('uses scope.data.intent when present (router-set), falls back to last user_message', async () => {
    ...
    ctx.events.push(
      toolResultEvent({ id: 'ev-1', patternId: 'p', tool: 'x', result: '1', summary: 's1' }),
    )
    ctx.events.push(
      toolResultEvent({ id: 'ev-2', patternId: 'p', tool: 'x', result: '2', summary: 's2' }),
    )
    const view = createEventView(ctx)
    const inner = makeInnerPattern()
    const wrapped = withReferences(inner.pattern, {
