/**
 * Tests for the non-agentic Neo4j query OPS (#225 PR-C2: moved co-located
 * with the module from the host app's `queries.test.ts`).
 *
 * The driver module is mocked, so these exercise the contract the host's
 * `'use server'` wrappers rely on: every function resolves to a
 * `{ success }` envelope instead of throwing, and the session is always
 * closed.
 *
 * Since #230 they also pin the module's read-only property at the DRIVER:
 * every session is a READ-mode one, so the driver — not a keyword blacklist
 * — is what makes the caller-supplied query read-only. The auth-gate half of
 * the old suite stayed in the host (it tests the retained wrapper); the ops
 * and the source-scan pins moved with the ops.
 */
...
import * as queries from '../../neo4j/queries'
import {
  getSchema,
  getSchemaForAgent,
  getSimplifiedSchema,
...
const queriesSource = () => readFileSync(new URL('../../neo4j/queries.ts', import.meta.url), 'utf8')
...
describe('runManualCypher', () => {
  ...
  // `'use server'` result — refuses *after* the headers are on the wire, so
  // the client sees a truncated stream instead of an error. Both halves of
  // the envelope are projected now (`neo4j/plain.ts`).
...
// Regression pin for #228: `executeWriteCypher(cypher)` used to live where
// this module's host wrapper does — browser-reachable, unauthenticated, and
// it ran whatever string it was handed. It is gone; graph writes go through
// the intent-shaped ops in `graph-edit.server.ts` (pinned by its own test).
...
describe('no arbitrary-Cypher write RPC (#228)', () => {
  it('is not exported from the module or the barrel', async () => {
    const barrel = await import('../../neo4j/index')
...
// Class pins for #230, held on the source rather than on one symbol: adding
// an export that opens a write-capable session fails here even if it never
// appears in a behavioural test. (The AUTH half of the old pin — every export
// gated on denyUnauthenticated — stayed with the host wrapper, which is what
// the gate now belongs to.)
describe('every op in this module is read-only (#230)', () => {
