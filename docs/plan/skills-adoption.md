...
- **Bare name** (`grilling`, `codebase-design`, `council`) — stack-agnostic
  procedure. No mention of Neo4j, BAML, SolidStart, UnoCSS, the company name, or DTalk. Ships
  with the OSS split.
...
| 8   | `wizard`                        | `engineering/wizard/`                        | **model**                                      | Copy `SKILL.md` (44 ln) **and `template.sh` (8.6 KB)** — the skill explicitly forbids hand-editing the library above the `STAGES` marker, so the template is not optional. No content edits. First real use: the manual Entra tenant checklist in `docs/deployment/entra-setup.md` (#119).                                                                                                                                                                                                                                                                                  | S      |
