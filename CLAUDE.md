...
Two things re-point part of that, and both move exactly the roles in `VERDA_CLIENT_BY_ROLE` — which since 2026-08-26 is every role except `screen`: controller / actor / critic / synthesizer, plus router / planner / describe. The three latecomers were held out on latency grounds until the owner overruled it — the router sees the user's raw message and describe is handed tool results verbatim, so excluding them left the private tier shipping exactly the payloads it exists to keep:
...
Both act through the SAME seam: a per-call `client` override spread into the BAML options bag. Re-pointing the chains in `baml_src/` is not the mechanism and must not become it — that class of edit moves whole roles at once, and `screen` must stay on `DescribeAnthropic` in every tier position (SA-M5). That is now the ONLY thing a tier decision leaves behind, so the trap is sharper than it was: `screen` and `describe` name the same chain in BAML, `describe` moves and `screen` does not, and the difference exists only in `clients.server.ts`.
