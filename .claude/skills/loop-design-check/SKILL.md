...
description: Write or review a goal-oriented agent loop — is the goal machine-decidable, and will the loop spin, game its own verifier, or run a wrong answer to completion. Use when designing an autonomous loop, or when you have one and worry it will run away.
---

<!-- Vendored from affaan-m/ECC (MIT © 2026 Affaan Mustafa), skills/loop-design-check/SKILL.md.
...
- *How to wire the loop architecture* (pipelines → DAGs, long-run recovery) → that's the mechanism layer, and it belongs to whatever runtime you are building on. **This skill only covers "is the goal right, and will it run away" — it does not explain mechanism.**
...
|---|---|---|
| Has a clear "done" test (write to done / a batch of images processed) | **servo** (closed loop onto a target) | stops on reaching the goal |
...
> This skill covers goal definition and runaway prevention only. How the loop is wired — the mechanism layer — is the runtime's business, not this skill's.
