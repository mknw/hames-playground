...
company tenant on **2026-08-03**, N=25 rows (10 driveItems, 15 mailbox attachments,
...
standard-base64 alphabet is _not_ evidence of an EWS id — we inferred that and
...
| `lastShared.sharedBy`          | the person who shared it _with you_ | The **actor** of the share, which is sometimes **you**. 3 of 25 rows named the signed-in user (2026-08-03), two of them files in their own OneDrive. A row's presence says the item was involved in recent sharing activity, not that it came inbound.                                                                                   |
...
Graph's rules, not ours. (For _why we sanitize_ what we send, see
...
| no status               | —                      | Token _acquisition_ failed before any HTTP call (MSAL `InteractionRequired`).                                                                                                     | Yes.                                                     |
...
token to it would leak the token. `Accept` _is_ forwarded, so content
...
- **Which direction `/me/insights/shared` actually covers.** Two samples of the
  ...
  rows shared _by_ the signed-in user, while 2026-08-03 (N=25) had **3** — a
...
- **How many mechanisms hide behind `sharingType: "Attachment"`.** Three
  distinct URL shapes were observed; attributing them to _specific_ user actions
