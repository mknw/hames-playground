...
describe('compactIntent', () => {
  ...
  it('exports a factory that returns a ConfiguredPattern', async () => {
    const { compactIntent, compactIntentFn } = await load()
    const pattern = compactIntent(compactIntentFn, { patternId: PATTERN_ID })
    ...
    const ctx = ctxOf([
      {
        ...
        ts: now,
        patternId: 'harness',
        data: { content: 'Write a Fibonacci script' },
      },
    ])
    const pattern = compactIntent(compactIntentFn, { patternId: PATTERN_ID })
    ...
    const ctx = ctxOf([
      {
        ...
        ts: now,
        patternId: 'harness',
        data: { content: 'Write a Fibonacci script to /work' },
      },
      {
        type: 'assistant_message',
        ts: now + 1,
        patternId: 'sandbox-session-synth',
        data: { content: 'Done, computed fib(10).', final: true },
      },
      {
        type: 'user_message',
        ts: now + 2,
        patternId: 'harness',
        data: { content: "I can't find the file" },
      },
    ])
    const pattern = compactIntent(compactIntentFn, { patternId: PATTERN_ID })
    const scope = createScope(PATTERN_ID, {})
