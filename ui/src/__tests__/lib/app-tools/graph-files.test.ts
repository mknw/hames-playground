...
describe('composeFileQuery', () => {
  ...
  it('adds filetype: for file_type', () => {
    expect(composeFileQuery({ query: 'budget', fileType: 'docx' })).toBe('budget filetype:docx')
  })
  ...
  it('combines all three, terms first', () => {
    ...
    ).toBe('q3 budget filetype:xlsx path:"https://contoso.sharepoint.com/sites/Finance"')
  })
...
describe('hostile input cannot restructure the query', () => {
  ...
  it('strips the operators out of free text instead of honouring them', () => {
    ...
    expect(kql).toBe('budget or filetype exe or path https //evil.example and size 0 not')
  ...
  it('leaves exactly one path: clause, with the attempt trapped inside its quotes', () => {
    ...
    expect(kql).toContain('path:"https://contoso.sharepoint.comORpath:https://evil.example"')
  ...
  it('keeps caller whitespace out of a filter clause (a space silently widens it)', () => {
    ...
    const kql = composeFileQuery({
      query: 'x',
      site: '  https://contoso.sharepoint.com/sites/A B  ',
    })
...
describe('cleanSummary', () => {
  it('strips every <cN> highlight marker, however high N goes', () => {
    expect(cleanSummary('<c0>a</c0> and <c1>b</c1> and <c12>c</c12>')).toBe('a and b and c')
  })
...
describe('webUrlFolderPath', () => {
  it('reads the containing folder out of a SharePoint webUrl, percent-decoded', () => {
    expect(
      webUrlFolderPath(
        'https://contoso.sharepoint.com/sites/Finance/Q3%20Reports/Q3%20Budget.xlsx',
      ),
