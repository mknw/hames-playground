...
**Scale-to-zero is a latency mode the whole app has to survive, not just the client.** Measured 2026-08-26 against the live deployment: a single completion into a sleeping box took **146s**; three chats sent in quick succession into a sleeping box are ONE replica's queue, and two of them were **still unanswered at the client's 600s `request_timeout_ms`** and ended as `BamlTimeoutError` — while the deployment's own gateway had already answered a sibling call with `504 {"error":"inference request was canceled"}` after **55s**. So the platform abandons a queued request long before this client does, twice per turn (controller, then synthesizer). Three consequences are now code rather than folklore:
...
- **The stream never goes byte-silent.** A blocked call writes nothing for its whole timeout, which no proxy with an idle read timeout (the preview runs behind Caddy) and no human can distinguish from a dead connection — the reload it provokes is what destroyed three of the four conversations that produced this note. `SSE_KEEPALIVE_MS` in `routes/api/events.ts` writes a comment frame every 15s; `parseChatStream` already dropped `:` lines, and `events-keepalive.test.ts` pins both halves.
- **What is still an owner decision:** the 600s timeout itself (it trades "survive a cold start" against "notice a cancelled request", and nobody has measured the platform's own ceiling), and the fact that a turn whose PROCESS dies leaves its row at `status='running'` forever — the row is seeded before the run and only rewritten at the end, so nothing reconciles an abandoned one. See the rationale block in `baml_src/verda-client.baml`.
