...
const cacheState = {
  ...
  getAllAccounts: vi.fn<() => Promise<FakeAccount[]>>(async () => [{ homeAccountId: 'hai-1' }]),
}
...
describe('graphFetch', () => {
  ...
  describe("responseType: 'base64'", () => {
    ...
    it('still maps 401/403 and 204 the same way', async () => {
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => ({ ok: false, status: 403, statusText: 'no' })),
      )
      await expect(
      ...
      await expect(graphFetch(USER, '/x/content', { responseType: 'base64' })).resolves.toBeNull()
    })
  })
})
