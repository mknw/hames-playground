...
| [`docs/ROUTINES.md`](docs/ROUTINES.md) | Routines (#131): trigger-driven harness runs. The trigger registry (`interval`/`session_start`/`session_end`, extensible without a migration), the `routines` table, the boot-armed sweep (`ui/src/middleware.ts`) + auth-route hooks, the `claimRoutineRun` compare-and-set, and the `/api/routines` management API. Runs go through the AGENT_TRIGGER path with `source='routine'` — never a second execution path |
