...
It is the one failure with the right shape, and the only one the other five
cannot see — see threat 6.
...
corpus, and no fix is proposed here.
...
1. **One model.** Only `claude-sonnet-5`, the lead of `SynthesizerAnthropic`.
   The Haiku 4.5 backstop is unmeasured. (The mixed-provider
   `SynthesizerFallback` chain this warning also named was removed on
   2026-08-24 — ADR-0001 — so only the Anthropic backstop remains unmeasured.)
   **Do not assume this result transfers to a smaller model.** If a smaller or
   non-Anthropic chain is ever
...
6. **The `unpresented` column was added after this run.** The scorer now also
   ...
   evidence; it is precisely what the guidance arm instructs the model not to do.
...
by at most one call (~$0.04) before it aborts. On abort the report is still
written, and the test then fails, which is the intended signal.

Every table in this document is emitted by the reporter — including the by-kind
