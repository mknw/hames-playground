...
is _under-using_ it (treating `code-mode-<name>` as a thin shim around a
...
capability does the JS _itself_ have (Node fs? V8 isolate? sandbox?) — is
...
- **Provenance** — if a result carries `_source`, the critic checks it came from
  ...
  user asked for neo4j) is rejected _with steering_ toward the right server, not
