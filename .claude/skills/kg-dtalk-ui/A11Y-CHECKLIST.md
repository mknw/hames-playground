...
| `reduced-motion`                | Apple/MD     | Wrap animation in `@media (prefers-reduced-motion: reduce)`. The preflight CSS already does this for `thread-flash-*` and `thread-progress-indeterminate` — copy that shape, and keep a motionless signal (border, dim fill) so the state is still communicated.    |
