...
function makeInner(): {
  pattern: ConfiguredPattern<Record<string, unknown>>
  ...
  it('postgres-18 (must select): "add this info to the graph" after web-search', async () => {
    ...
    ctx.events.push(
      toolResultEvent({
        ...
        summary: 'Postgres 18 release notes and feature overview',
        ts: 3,
      }),
    )
    ...
    const fixtureSelector: SelectorFn = async (input) => {
      ...
      return pg18
        ? {
            reasoning: 'pg18 ref directly answers "this info"',
            selected: [{ ref_id: pg18.ref_id, reason: 'subject of prior turn' }],
          }
    ...
    const scope: PatternScope<Record<string, unknown>> = {
      id: 'wrap-neo4j',
      data: { intent: 'add this info to the graph' },
      events: [],
      startTime: Date.now(),
    }
  ...
  it('conversational-unrelated: "thanks!" yields empty selection', async () => {
    ...
    const fixtureSelector: SelectorFn = async (input) => {
      ...
      return conversational
        ...
        : {
            reasoning: 'fallback include all',
            selected: input.candidates.map((c) => ({ ref_id: c.ref_id, reason: 'fallback' })),
          }
    }
  ...
  it('multiple-relevant: "summarize what we found" selects all 3 within budget', async () => {
    ...
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'fetch',
        result: 'A',
        summary: 'page about kubernetes networking',
        ts: 1,
      }),
    )
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'fetch',
        result: 'B',
        summary: 'page about kubernetes pod lifecycle',
        ts: 2,
      }),
    )
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'fetch',
        result: 'C',
        summary: 'page about kubernetes RBAC',
        ts: 3,
      }),
    )
    ...
    const scope: PatternScope<Record<string, unknown>> = {
      id: 'wrap',
      data: { intent: 'summarize what we found' },
      events: [],
      startTime: Date.now(),
    }
  ...
  it('scope=self: only own-pattern refs reach the selector regardless of relevance', async () => {
    ...
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'fetch',
        result: 'unrelated',
        summary: 'web page about Person schemas',
        ts: 1,
      }),
    )
    ctx.events.push(
      toolResultEvent({
        id: 'ev-neo-1',
        patternId: 'wrap-neo4j',
        tool: 'read_neo4j_cypher',
        result: 'rows',
        summary: 'prior neo4j Person query',
        ts: 2,
      }),
    )
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'read_neo4j_cypher',
        result: 'rows',
        summary: 'prior neo4j relationship walk',
        ts: 3,
      }),
    )
    ...
    const scope: PatternScope<Record<string, unknown>> = {
      id: 'wrap-neo4j',
      data: { intent: 'list all Person nodes' },
      events: [],
      startTime: Date.now(),
    }
  ...
  it('stale-on-topic: prefers more-recent items when relevance is similar', async () => {
    ...
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'read_neo4j_cypher',
        result: 'rows',
        summary: 'older neo4j result on Person',
        ts: now - 600_000,
      }),
    )
    ctx.events.push(
      toolResultEvent({
        ...
        tool: 'read_neo4j_cypher',
        result: 'rows',
        summary: 'recent neo4j result on Person',
        ts: now - 5_000,
      }),
    )
    ...
    const fixtureSelector: SelectorFn = async (input) => {
      ...
      return {
        reasoning: 'recency tie-break',
        selected: sorted
          .slice(0, 1)
          .map((c) => ({ ref_id: c.ref_id, reason: 'most recent on-topic' })),
      }
    }
