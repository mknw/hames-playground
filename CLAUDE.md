...
**Wrong envelope, right action:** a third recoverable parse cause, and the only one where the model was not wrong about the work. Captured live (`.harness-logs/baml-validation.json`): the sandbox actor wrote a COMPLETE action — reasoning, tool, escaped `tool_args`, a second call in `additional_calls`, status, `is_final` — as brace-less `key: value` lines, because its own few-shot section rendered examples in exactly that shape. 372 output tokens against a 32768 cap and a non-empty response, so neither existing retry trigger matched; the throw killed the actorCritic loop with three attempts still in budget. Fixed at both ends: `ActorFewShots` / `LoopFewShots` now render the JSON object envelope (fourth instance of "disagreeing demonstrations are defects" — see the comment on `LoopFewShots`), and `coerceControllerActionText` (`controller-action.ts`) recovers the shape in the adapters BEFORE the truncation branch, since no retry can improve on an action that is already correct. It declines anything it cannot read completely OR cannot read unambiguously — a partly-readable `additional_calls` (including an entry whose `tool_args` is absent, empty or a bare scalar), an unterminated literal, a second COLUMN-0 `tool_name`/`tool_args`/`additional_calls`/`is_final`, or a call-deciding field line anywhere else in the text at ANY indentation outside the values already consumed — so a half-batch is never executed and an echoed tool result never gets to choose the call. That last clause is what makes the claim hold regardless of layout: counting column-0 duplicates alone left the inverted case (the echo at column 0, the model's own envelope indented) with a single unopposed candidate, and it is the sandbox agents — no `withInjectionGuard` on tool results, `sandbox_bash` allowlisted — that this recovery exists for. Only column 0 is read as top level, so a uniformly indented envelope declines rather than recovering; that is deliberate, since reading fields at arbitrary indentation is precisely what makes the two blocks indistinguishable. `is_final` is in the decline set because simpleLoop breaks on it BEFORE executing the turn's tool and `normalizeControllerAction` only fills an ABSENT value; `reasoning` and `status` stay first-wins (prose, and display-only). Truncation is decided by the collector's cap verdict, NOT by the lexical unterminated-literal signal: the recovery declines before it reads the text at all unless the collector PROVES the response finished under a KNOWN cap, because a cut can land inside a QUOTED `tool_args` or between two `additional_calls` items and leave nothing unbalanced to see. "Cap unknown" therefore declines too — no collector, or a client with no `CLIENT_MAX_OUTPUT_TOKENS` entry (the OpenAI leaves, i.e. `ControllerFallback`'s fourth rung under `USE_MIXED_CHAINS=1`) — which falls through to the existing retry/Groq escalation rather than recovering a silently short batch; inventing a cap number for those leaves would be a guess wearing a measurement's clothes. The 372/32768 capture is unaffected; a genuine cap-hit stays on the truncation retry.
