...
- **The BAML client's timeout dropped 600s → 120s**, which is the payoff. The case FOR 600s was only ever "a cold start must be able to finish", and nothing pays a cold start there any more; the case against it — ten minutes in which a silently dropped request is indistinguishable from a working one — is untouched. 120s is ~12x the measured warm p95 (9.9s at 8-way concurrency, 2026-08-25), above the 55s at which the platform's own gateway was once seen to cancel (so a prompt 504 still surfaces as a 504), and below the wake's own bound. **The ~150s hard floor moved with the wait**: it now binds `VERDA_WAKE_TIMEOUT_MS` (300s), not this client.
- **A wake that fails ends the turn VISIBLY** — an `error` frame saying the private inference box did not wake, and that nothing was sent to any other provider. It does not proceed (same wait, now against a 120s timeout) and it does not fall back to Anthropic. The wake's duration is also what feeds the cold-start estimate, through the same `settleColdStart` funnel and therefore the same two gates; only the turn that STARTED the ping records, because a turn that arrived 100s into a 146s wake waited 46s — which clears the plausibility floor and is a fragment of one wait rather than a measurement of anything.
...
- **What is still an owner decision:** the timeout question is settled at 120s by wake-then-run (above), so what remains open is narrower. `VERDA_WAKE_TIMEOUT_MS` is 300s against cold starts of 146s and 71.7s, which is a judgement about a tail nobody has measured (and the 146s UI fallback is deliberately NOT averaged down by the faster reading — erring long costs a pessimistic countdown, erring short is a promise the box breaks). Pricing the 4B is unresolved, so a private-tier step that summarized a tool result reads cost-unknown rather than cheap. And a turn whose PROCESS dies still leaves its row at `status='running'` forever — the row is seeded before the run and only rewritten at the end, so nothing reconciles an abandoned one. See the rationale block in `baml_src/verda-client.baml`.
