...
async function createPatterns(sessionId: string): Promise<ConfiguredPattern<SessionData>[]> {
  ...
  const routesPattern = routes<SessionData>({ code_mode: codeChain }, { liveEvents: true })
...
export const codeModeAgent: AgentConfig = {
  ...
  description: 'Orchestrate multiple MCP tools via JavaScript scripts run by the kg-agent gateway',
